...
def crawl(tile_file,config_file=None,change_driver=False):
    '''
    crawl tiles from given tile file, config file (optional, which overrides the contained configs), and change_driver optional
    ...
    if len(args)<=1:
        ...
        if mode=='-c':
            if len(param)<=2:
                ...
                crawl(param[0],param[1],bool(param[2]))
        elif mode=='-t':
